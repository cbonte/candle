...
static void instantiate_mmq_q8_0(float * tmp_fixup,
    ...
    constexpr int qk_t = ggml_cuda_type_traits<GGML_TYPE_Q8_0>::qk;
    ...
    if (args.nrows_x % mmq_y == 0) {
        mul_mat_q<GGML_TYPE_Q8_0, mmq_x, false><<<grid_sk, block_dims, nbytes_shared, stream>>>(
            ...
            ntx_fd);
        if (fixup_needed) {
            mul_mat_q_stream_k_fixup<GGML_TYPE_Q8_0, mmq_x, false><<<grid_sk_fixup, block_dims_fixup, 0, stream>>>(
        ...
        mul_mat_q<GGML_TYPE_Q8_0, mmq_x, true><<<grid_sk, block_dims, nbytes_shared, stream>>>(
            ...
            ntx_fd);
        if (fixup_needed) {
            mul_mat_q_stream_k_fixup<GGML_TYPE_Q8_0, mmq_x, true><<<grid_sk_fixup, block_dims_fixup, 0, stream>>>(
                args.ids_dst, args.expert_bounds, args.dst, tmp_fixup, blocks_per_ne00_fd, args.nrows_x, args.ncols_dst,
                args.nrows_dst, nchannels_y_fd, args.stride_channel_dst, nsamples_y_fd, args.stride_sample_dst,
                ntx_fd);
        }
